...
uint8_t chache[CACHE_LINE_CAP][LCD_WIDTH + 1]; // The chach size is CACHE_LINE_CAP lines of LCD_WIDTH + 1 length (Each
                                               // line is an string, and string ends up to '0').
...
uint32_t last_line(void) { return (cache_start + cache_size) % CACHE_LINE_CAP; }

uint32_t last_window_line(void) { return (window_start + window_size) % CACHE_LINE_CAP; }
